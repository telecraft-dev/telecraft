...
configuration from owned, versioned building blocks, and then checks the
result: from the configuration it derives an *expectation* of what telemetry
should arrive, and verifies that it did. Green means "the configuration
worked", never only "the configuration applied".

See it running: **[demo.telecraft.dev](https://demo.telecraft.dev)** is the
real console over a public demo estate. It is read-only and built from the
latest release, not from `main`, so between releases it shows what you can
run today rather than what is coming.

Telecraft has three parts. Adopt any of them, in any order:
...
| **Serving** | A stateless OpAMP server that delivers rendered configuration from git to collectors, or GitOps instead, chosen per collector | An OpAMP Supervisor beside each served collector |

Five principles hold everywhere:

- **Nothing sits in the telemetry path.** If Telecraft is down, telemetry
  keeps flowing.
- **Git is the source of truth.** History, rollback, approval, and audit are
  git's, not Telecraft's.
- **Configurations, never binaries.**
- **Vendor-neutral core.** Elasticsearch, Prometheus, and the rest are
  plugins.
- **Air-gapped deployment is first class.** There is no hard dependency on
  any hosted service.
...
If you want to contribute, [contributing](docs/contributing/) explains how
...
| `tools/vendorlint/` | The vendor-word lint; its scope globs in [`vendorlint.yaml`](vendorlint.yaml) are the core and provider boundary |
| `docs/` | Documentation, and the design records behind it |
...
Everything a user sees (console surfaces, documentation, `telecraft.dev`, and
CLI output) follows one design system.
...
and the marketing site as well as this one.
