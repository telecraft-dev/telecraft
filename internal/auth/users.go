...
func LoadUsers(path string, tree ownership.Tree) (Users, error) {
	...
	if doc.Kind != yaml.DocumentNode || len(doc.Content) == 0 {
		return Users{}, fmt.Errorf("%s: empty file — the users seam holds at least one user", path)
	}
	...
	if len(file.Users) == 0 {
		return Users{}, fmt.Errorf("%s: holds no users — an instance nobody can sign in to serves nothing", path)
	}
	...
	for _, u := range file.Users {
		...
		switch {
		case email == "":
			problems = append(problems, "a user has no email — email is the join key from the authenticated identity")
			continue
		...
		if _, dup := users.byEmail[email]; dup {
			problems = append(problems, fmt.Sprintf("user %q appears twice — one email, one user", email))
			continue
		}
		if u.Name == "" {
			problems = append(problems, fmt.Sprintf("user %q has no name — a nameless identity cannot author a change (ADR-0019 §3)", email))
		}
		if u.Owner == "" {
			problems = append(problems, fmt.Sprintf("user %q names no owner — without one, nothing derives what they may author (ADR-0016)", email))
