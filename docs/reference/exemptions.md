...
Service or one Team subtree, with a mandatory owner and a mandatory expiry.

An Exemption waives the count, never the diagnosis. A waived finding keeps its
outcome and its detail in the report and gives up only its contribution to the
exit code, so a green built on exemptions can't look like a clean green.
Nothing here forbids complying: there are no narrowing semantics.
...
The decision behind the format is ADR-0037.
...
| `owner` | string | yes | none | The party answering for the waiver. |
| `expires` | date | yes | none | Authored as `2026-09-01`. The waiver stops counting at the UTC midnight starting that day. |
| `service` | string | one of two | none | The subject: one Service. |
| `team` | string | one of two | none | The subject: one Team subtree. |
| `reason` | string | no | empty | Why the waiver exists. Appended to the waiver reason in the report. |

Exactly one of `service` and `team` must be set. Setting neither or both is a
load error.
...
`team` names a Team and covers its whole subtree. That's the onboarding case:
one reviewable file rather than a copy per Service.

Resolving a team-scoped Exemption needs the ownership model, because it has to
answer whether a Service belongs to the named Team's subtree. `telecraft
check` wires that from `-ownership`. Without it, a team-scoped Exemption is an
error on the run rather than a waiver that silently never applies. A Service
the ownership model doesn't know sits provably in no subtree, so the waiver
stays unapplied and the finding stays visible.

## Expiry

Expiry is a property of the clock alone. There's no manual step: an expired
Exemption stops matching, so the raw finding is back on the next run.

Renewal is a fresh change to the file. An expired Exemption still in the tree
is dead config, and raises an [authoring finding](#authoring-findings).

## Owner review

The validity rule is that the change adding an Exemption must be approved by
the waived Requirement's owner, or by that owner's ancestor team. That's
enforced by the generated code-ownership projection, not by this loader, which
makes self-forgiveness impossible by construction rather than by policy.

## Precedence with Grace

Two things can waive a finding's count on one run: an authored Exemption, and
the Grace Period the platform computes from a Service's Class and onboarding
date. They stay distinct, and an Exemption wins where both cover the same
finding, because it names the party answering for the waiver.
...
Exemption id, its owner, its expiry date and its reason; the waiver reason for
Grace names the Service Class and when the onboarding window ends.
...
The table is authored highest class first, and grace shrinks as class rises,
so windows must never shrink going down the list. A table that quietly gave
the most critical class the longest forgiveness can't load.
...
| `services[].onboarded` | date | no | Authored as `2026-08-01`. |
...
The window runs from the onboarding date for the class's duration; outside it,
including before onboarding, nothing is waived.

An authored row is a known reading by definition, including one reporting no
pipelines at all: that's a collector reporting an empty config, not a blind
spot.
...
an empty library would pass everything vacuously, but zero exemptions is the
strictest state there is.
...
- an unknown or misspelled field
- no `services` at all: an empty estate would pass every check vacuously
...
carrying both the Exemption id and the Requirement it names, and are never
part of the exit code.

Two are raised:

- The Exemption waives a Requirement the library doesn't hold. It waives
  nothing, and it's almost always a typo whose author believes a waiver is in
  force.
- The Exemption has expired and is still in the tree. Renewal is a fresh
  change; otherwise delete the file.
