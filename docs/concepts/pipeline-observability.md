...
This is the part that makes green mean "the config worked" rather than "the
config applied". From the Intended configuration at a commit SHA, Telecraft
derives what telemetry should arrive, and then checks whether it did.

## Expectations

An **Expectation** is a derived, never-authored, checkable statement about
what should be observable, computed from the Intended configuration at a
commit SHA.

Three separate reds live side by side and must never be blended:
...
An Expectation is also not a Requirement. A Requirement is what a human
...
That restraint is deliberate. A curated model of component behaviour would
widen coverage, and it is a named post-v1 extension rather than a v1 feature,
because one false expectation-red would poison trust in the whole band.

### Derived at evaluation time, never committed

No expectations file exists. Derivation runs at evaluation time as a pure
function of the artefact, memoised in memory by SHA and confirmed loseable.

A materialised copy would be a drift surface against the artefact it restates,
and a committed file would invite authored semantics by social pressure.
Derived-never-authored is easier to defend when the claim set never looks like
a file.
...
This makes delivery failure structurally unable to cascade into
expectation-red: an unapplied configuration's claims are never in force, and
the running artefact's claims still are.

The consequence is worth naming. A collector pinned to an ancient artefact is
judged green against ancient claims. That is correct band separation: the
staleness is delivery's red, and it must never be "fixed" by blending the two.
...
far back a claim looks, defaulting to 24 hours, generous enough to survive an
overnight quiet period. Shortfalls are dampened by persistence, using the same
...
- **Requirement-backed data claims** raise no finding of their own. They are
  the machinery behind the cross, and the Requirement world already owns their
  severity and routing.
- **Unbacked data claims**, where the configuration implies a signal no
  Requirement demands and it never arrives, raise a Service-attached,
  `expectation`-kind finding that is advisory grade and never a violation. No
  human demanded the signal, so it cannot fail compliance. The remediation
  text is honest about the fork: fix the pipeline, or delete the dead lane.
  That doubles as dead-configuration detection.
- **Pipeline claims** raise Tier-attached findings routed to the Tier's owner
  that *can* reach violation grade once dampened. An instantiated exporter at
  100% send failure is "the config didn't work" in its sharpest form.

The asymmetry is deliberate: a dead lane may be intentional, whereas a failing
instantiated component never is.
...
Claims about pipelines need the collector's own health data, and the platform
...
Getting there takes a rendered pattern, because the collector's defaults will
not do it. The default surface is a localhost Prometheus endpoint nobody
scrapes.

**Every rendered artefact configures OTLP push of internal telemetry**,
metrics at level `normal` through a periodic reader and internal logs through
a batch processor, to the self-telemetry destination. Internal traces stay off.
...
self-telemetry only, and a governed Tier reporting less than the ungoverned
fallback would be absurd. The ingest volume this creates is a real, named cost.

The destination is declared once, estate-level, and resolved per Tier at
render. One routing rule governs it: **a Tier's self-telemetry never depends on
that Tier's own data pipelines.** A gateway does not export its own health
through itself. An edge Tier whose only network path runs through a gateway
may transit it as ordinary data, and the shared fate is topological reality
rather than a design flaw: transport loss reads as `Known: false`, never red.

The renderer owns the telemetry block. Blueprints do not author it.

### Joining readings back to configuration

One platform-owned normalisation layer maps the identity attributes a reading
carries back to the component in the rendered YAML, keyed by kind and id.
Both generations of join key meet there and nowhere else:
...
failures: synthetic graph nodes, singleton components that deliberately drop
identity attributes, two-level scraper identity, and a prefix trap between two
similarly named attributes. Receivers and exporters carry no pipeline id on
any surface, so pipeline membership is derived from the rendered configuration
topology rather than from telemetry.

One collector process start is an **Incarnation**, identified by
`service.instance.id`. Its rotation on every restart is kept deliberately,
because incarnation churn per Tier is the restart-rate reading.
...
authored to do. "Loss" is not vocabulary here, because the word for the same
number that implies fault would make every correctly authored filter look like
one.

Negative reductions are real and stay negative: a connector, or a fan-out to
two exporters, honestly sends more items than the receivers accepted.
...
reading behind it, so surfaces render last-known-plus-age rather than a
confident zero.

The scaling argument that makes on-read tenable: query cardinality follows
*authored* objects, Tiers by components by signals, not collector count.
An estate of 22,000 collectors collapses into server-side sums. The named cost
is console latency as a function of your backend's query speed, accepted
rather than building a shadow metrics store.
...
- **Three bands in fixed order**: Delivery, Expectation, Conformance. Each
  band is an *enum state*, plus an optional worst-finding label. States include
  the honest neutrals, each distinct: not applicable, unknown, pending settle,
  and stale demoted.
...
Colour appears nowhere in the contract. States are the contract and glyphs map
from states, which enforces structurally that hue is enhancement rather than
load-bearing: a renderer that cannot show colour still shows three distinct
readings.
...
face carries a provenance object tracing it back to the configuration lines
that implied it and the SHA judged against. A finding without remediation is a
complaint, and derived values do not explain themselves.

Reference: [ADR-0038](../adr/0038-expectation-engine.md),
[ADR-0039](../adr/0039-self-telemetry-ingestion.md),
[ADR-0040](../adr/0040-metering-derived-flow-readings.md),
[ADR-0041](../adr/0041-card-data-contract.md).
