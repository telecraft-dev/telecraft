...
| **Placement** | Where a `schema_conformance` Requirement is checked. `landed`, the default, checks telemetry that has landed in a backend. `live` is declared and refused: a library that sets it does not load. |
| **Live-check tap** | The collection-time source of schema conformance findings that `placement: live` names. Telecraft has none: a Requirement that sets `placement: live` is refused at load. |
