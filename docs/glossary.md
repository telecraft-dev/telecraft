...
| **Environment lens** | The control in the strip above every console Workspace that picks which Environment leads, `production` by default. It sets emphasis and evaluation context, not a filter: surfaces that show several Environments keep every row visible. Your choice is remembered, and a lens in a URL overrides it. |
