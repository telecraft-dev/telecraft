...
| Go formatting (issue #146) | `go run ./tools/fmtlint` | Every tracked Go file is `gofmt` clean, naming the ones that are not. `go build`, `go vet` and `go test` never read layout, so an unformatted file otherwise passes every check the repository has. |
