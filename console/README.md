...
| `/api/v1/edition` | Which Edition this Instance is running, and the one quiet line the chrome shows for it under the version (ADR-0070 §5): `Standard Edition`, or `Enterprise Edition, licensed to Acme Ltd, expires 3 March 2027`. The line is composed server-side so the console, the CLI and the operator's terminal cannot disagree; the console shows it and never assembles one of its own. It is a fact about the reader's session, never a health signal, and it carries no refusal: a surface that withholds something says so itself. |
